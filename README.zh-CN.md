...
> **`E3` —— 方法验证，不是企业诊断。** 没有发生过客户委托。该记录是记录者对自己亲手执行过的工作的事后复盘，并已脱敏。它支持的是**方法表现如何**，不是**任何企业如何运作**。

本仓库其他案例都是合成的。[`case-studies/real-process-retrospective.md`](case-studies/real-process-retrospective.md) 是第一个来自**真实工作**的案例，它存在的原因是合成案例有一个本 README 已经承认的弱点：**当出题人和答题人是同一个人时，只能证明方法自洽。**

把它用在真实内容上之后，产出了这些：

- **一个由方法自己抓到的范围错误。** 估算工作量时发现工作单元选错了 —— 同一组操作在另一个频率高得多的触发源下运行。如果只盯着原本选定的范围，会得出「这事不重要」。
- **两个 AI 推断：流畅、看似合理、并且都错了。** 两条都已写进草稿。它们之所以被拦下，只因为被标注为推断并逐条核对。若被写成事实，一份真实案例里现在已经有两处假事实。**这就是「AI 生成的流畅不等于证据」的具体形态。**
- **三条规则升入权威证据模型** —— 包括「口头声明的规则在没有文件时停留在 `E3`」，以及一个**尚未解决**的缺口：模型无法确认一份上传文件**确实是**企业制度文件。
- **一条被故意扣下的洞察。** 一个看起来很强的判断只基于单个案例，项目自己的元规则因此禁止它进入核心。它被记入 [`CHANGELOG.md`](CHANGELOG.md)，注明已识别、暂不采纳及原因 —— 而不是被悄悄用掉或悄悄扔掉。
- **一个原设计遗漏的方法限制。** 「最小补证请求」假设当事人仍能接触到材料、同事和系统。对已离开岗位者的复盘，所有补证路径会同时失效，因此该案例永久停留在 `E3`。

该页明确写出它省略了什么以及为什么。它比一份客户案例声称得更少 —— 那正是方法在起作用。
