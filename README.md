...
Two further discrepancies are worth stating rather than smoothing over. Concrete pilot artifacts in this repository are **5-day** runs, while the specification says 3–7 days and one blocked-path document says 3–5 days; the day counts are not consistent. And `outputs/crm-shadow-pilot-20260816/crm-shadow-pilot-result.xlsx` is a committed XLSX with seven named sheets for which **no generating script exists in this repository** — its declared result fingerprint cannot be recomputed from the tree.
...
- **Not CI-tested.** There is no `.github/workflows/`. Nothing validates this repository automatically on push.
- **Not released.** No git tags, no GitHub Releases, no `CHANGELOG`. There is no installable package and no published version to depend on.
...
`pytest` is not used and is not required; every suite runs on stdlib `unittest`. There is no single root test runner, and no CI runs these commands for you.
