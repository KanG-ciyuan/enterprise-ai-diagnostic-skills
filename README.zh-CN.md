...
还有两处不一致值得直说，而不是抹平。仓库中具体的试点产物都是 **5 天**运行，而规范写的是 3–7 天，另一份受阻路径文档写的是 3–5 天；天数口径并不统一。另外 `outputs/crm-shadow-pilot-20260816/crm-shadow-pilot-result.xlsx` 是一份含 7 个工作表的已提交 XLSX，但**仓库中不存在生成它的脚本**——它声明的结果指纹无法从当前代码树重新计算。
...
- **没有 CI。** 仓库没有 `.github/workflows/`。push 之后没有任何东西会自动验证这个仓库。
- **没有发布。** 没有 git tag、没有 GitHub Releases、没有 `CHANGELOG`。没有可安装的包，也没有可依赖的已发布版本。
...
不使用也不要求 `pytest`；所有套件都跑在标准库 `unittest` 上。仓库没有统一的根级测试入口，也没有任何 CI 替你跑这些命令。
